...
export default function CreateResultPage() {
  ...
  const [classicScorers, setClassicScorers] = useState<any[]>([]);
  ...
  const updateScorer = (team: 'classic' | 'away', index: number, field: string, value: string) => {
    const update = team === 'classic' ? [...classicScorers] : [...awayScorers];
    update[index][field] = value;
    team === 'classic' ? setClassicScorers(update) : setAwayScorers(update);
  };

  const generateScorerFields = (team: 'classic' | 'away', score: number) => {
    const current = team === 'classic' ? classicScorers : awayScorers;
    const updated = Array.from({ length: score }, (_, i) => current[i] || { name: '', minute: '', type: 'Goal' });
    team === 'classic' ? setClassicScorers(updated) : setAwayScorers(updated);
  };
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (classicScorers.length !== parsedHome) return toast.error("Classic FC scorers count mismatch");
    ...
    try {
      await API.post('/results/', {
        ...
        classic_scorers: classicScorers,
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <input
                ...
                onChange={(e) => {
                  handleChange(e);
                  generateScorerFields('classic', parseInt(e.target.value || '0'));
                }}
            ...
            {classicScorers.length > 0 && (
              <div>
                <h4 className="font-semibold mt-4 mb-2">Classic FC Scorers</h4>
                {classicScorers.map((s, i) => (
                  <div key={i} className="grid grid-cols-3 gap-2 mb-2">
                    <input
                      ...
                      className="border p-2 rounded"
                      value={s.name}
                      onChange={(e) => updateScorer('classic', i, 'name', e.target.value)}
                      required
                    />
                    <input
                      type="number"
                      ...
                      className="border p-2 rounded"
                      value={s.minute}
                      onChange={(e) => updateScorer('classic', i, 'minute', e.target.value)}
                      required
                      min={0}
                    />
                    <select
                      className="border p-2 rounded"
                      value={s.type}
                      onChange={(e) => updateScorer('classic', i, 'type', e.target.value)}
                    >
